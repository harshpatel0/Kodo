...
Universal fallback input — works on any app, including ones incompatible with UIA (canvas elements, focus-required JS listeners, unsupported control types). Only use when Direct App Control cannot handle the interaction. Tradeoff: steals focus and takes over mouse/keyboard, so the user cannot use their PC while it runs.
...
{"action": "scroll_v", "amount": int, "x": int, "y": int, "history": "string"}
{"action": "scroll_h", "amount": int, "x": int, "y": int, "history": "string"}
```
